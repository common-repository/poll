<?php
function wpacp_admin_add_poll() {
	global $wpacp_options;

	if( isset($_POST['wpacp_add_submit']) ) {
	
		global $wpdb,$table_prefix,$wpacp_options;
	...
	//for questions
		...
		for($i=0;$i<$j;$i++){
			...
			if( !empty($qtitle) ){
				$sql="INSERT INTO ".$table_prefix.WPACP_QUESTIONS_TABLE." ( pollq_pid, pollq_order, pollq_title, pollq_desc, pollq_optional) VALUES ('".$pollid."','".$i."' ,'".$_POST['pollq']['title'][$i]."', '".$_POST['pollq']['desc'][$i]."', '".$_POST['qopt'][$i]."')";
				$wpdb->query($sql);
			}
		}
	...
	//for answers
		...
		for($i=0;$i<$j;$i++){
			$sql="INSERT INTO ".$table_prefix.WPACP_ANSWERS_TABLE." ( polla_pid, polla_order, polla_bg, polla_fg, polla_answer) VALUES ('".$pollid."','".$i."' ,'".$_POST['polla']['bg'][$i]."', '".$_POST['polla']['fg'][$i]."', '".$_POST['polla']['text'][$i]."')";
			
			$wpdb->query($sql);
		}
		$edit=$pollid;
	}
	elseif( isset($_POST['wpacp_edit_submit']) ){
		global $wpdb,$table_prefix,$wpacp_options;
	
		$poll_table = $table_prefix.WPACP_POLLS_TABLE;
		$questions_table = $table_prefix.WPACP_QUESTIONS_TABLE;
		$answers_table = $table_prefix.WPACP_ANSWERS_TABLE;
		...
		if($qdel_arr){
			foreach($qdel_arr as $qid){
				if( $qid != '0' ) {
					$sql = "DELETE FROM ".$table_prefix.WPACP_QUESTIONS_TABLE." where pollq_qid = '".$qid."'";
					$wpdb->query($sql);
				}
			}
		}
...
<div class="wrap" id="addform">
	<h2><?php _e('Add Colored Poll','wpacp'); ?></h2>
	<!--<form name="add_poll" id="add_poll" action="<?php echo wpacp_plugin_url('admin/addSubmit.php');?>" method="post">-->
...
<div style="float:left;width:70%;">	
	<form name="add_poll" id="add_poll" action="<?php menu_page_url('wpacp-add-poll');?>" method="post">
	<table class="form-table">
		<tbody>
		<tr valign="top">
			<th scope="row" style="font-weight:bold;width:100px;"><label for="pollp_title"><?php _e('Title','wpacp');?> <em>*</em></label></th>
			<td><input type="text" size="50" id="pollp_title" name="pollp_title" value="" class="required" placeholder="<?php _e('Your Title...','wpacp');?>" /></td>
		</tr>
		<tr valign="top">
			<th scope="row" style="padding-top:40px;font-weight:bold;width:100px;"><label for="polldescedit"><?php _e('Description','wpacp');?></label></th>
			<td>
	...
	<table class="wpacpblock"></table>
	<h3><?php _e('Questions','wpacp'); ?></h3>
	
	<table class="form-table" id="pollq_add_table" style="width:380px;">
		<tbody id="pollq_add_tbody">
		
		<?php for($i=0;$i<3;$i++) { ?>
		<tr valign="top" style="cursor:move;" id="rq<?php echo $i;?>"><td style="" ><input style="width:100%;" type="text" size="50" name="pollq[title][]" class="pollq_title" value="" title="Title..." autocomplete="off" /> <br /><textarea style="width:100%;" rows="2" cols="50" name="pollq[desc][]" class="pollq_desc"  title="Description..." autocomplete="off"></textarea> <br /><input type="hidden" id="qopt-<?php echo $i;?>" name="qopt[]" value="0" /><input class="qoptck" id="qoptck-<?php echo $i;?>" type="checkbox" name="pollq[optional][]" value="0" checked /> <label for="qoptck-<?php echo $i;?>"><?php _e('Mandatory','wpacp'); ?></label></td><td class="dragHandle" title="Drag to reorder" >&nbsp;<input type="hidden" name="qorder[]" value="<?php echo $i;?>" /></td><td style="padding-left:2px;"><input style="text-indent:-9999px;" type="button" id="q<?php echo $i;?>" value="0" class="deleteRow" onclick="wpacp_edit_delete_question_js(this.id);" /> <input type="hidden" value="0" name="qid[]" /></td></tr>
		<?php } ?>
	</tbody></table>
	
	<p>
			<input type="button" class="button" onclick="wpacp_add_poll_question_js('3');" value="<?php _e('Add question','wpacp'); ?>" /> &nbsp; </p>
	
	
	<table class="wpacpblock"></table>
	<h3><?php _e('Answers','wpacp'); ?></h3>
	
	<table class="form-table">
		<tr valign="top"> 
			<th scope="row" style="width:130px;"><label for="pollp_aorder"><?php _e('My answers start by','wpacp'); ?></label></th> 
			<td><input id="pollp_aorder0" name="pollp_aorder" type="radio" value="0" <?php checked('0', '0'); ?>  /> <label for="pollp_aorder0"><?php _e('negatives and end with positives','wpacp'); ?></label><br />
			<input id="pollp_aorder1" name="pollp_aorder" type="radio" value="1" <?php checked('1', '0'); ?>  /> <label for="pollp_aorder1"><?php _e('positives and end with negatives','wpacp'); ?></label>
			</td> 
		</tr>
	</table>
	...
	<table class="form-table" id="polla_add_table" style="width:320px;margin-left:10px;">
		<tbody id="polla_add_tbody">
			...
			<input type="button" class="colorbox color_bg<?php echo $i;?> colorbox_bg<?php echo $i;?>" id="colorbox_bg<?php echo $i;?>" style="background-color:<?php echo $wpacp_options['colors'][$i]['bg'];?>;width:20px;height:20px;" /> <input name="polla[bg][]" type="hidden" id="bg<?php echo $i;?>" value="<?php echo $wpacp_options['colors'][$i]['bg'];?>" /> &nbsp;
			
			<input type="button" class="colorbox color_fg<?php echo $i;?> colorbox_fg<?php echo $i;?>" id="colorbox_fg<?php echo $i;?>" style="background-color:<?php echo $wpacp_options['colors'][$i]['fg'];?>;width:20px;height:20px;" /> <input name="polla[fg][]" type="hidden" id="fg<?php echo $i;?>" value="<?php echo $wpacp_options['colors'][$i]['fg'];?>" /> &nbsp;
			
			<input name="polla[text][]" size="25" type="text" class="color_bg<?php echo $i;?> color_t<?php echo $i;?> atext" style="background-color:<?php echo $wpacp_options['colors'][$i]['bg'];?>;color:<?php echo $wpacp_options['colors'][$i]['fg'];?>;" value="<?php echo $wpacp_options['colors'][$i]['text'];?>" autocomplete="off" />
			...
			<td style="padding:0px;padding-left: 2px;"><input type="button" id="a<?php echo $i;?>" class="deleteRow" onclick="wpacp_delete_answer_js(this.id);" /></td>
			</tr>
			<?php } ?>	
		</tbody>
	</table>
	...
	<p>	
			<input type="button" class="button" name="add_answer" onclick="wpacp_add_poll_answer_js('5');" value="<?php _e('Add answer','wpacp') ?>" /> 
	</p>
    <p>	<input type="button" class="button" name="reset_colors" value="<?php _e('Reset Colors','wpacp') ?>" onclick="wpacp_reset_colors_js(<?php echo $i;?>);" /> 
	</p>
	...
	<table class="form-table">
		<tr valign="top"> 
			...
			<td valign="middle"><?php _e('Color of unanswered question','wpacp'); ?>
			</td> 
		</tr>
	</table>
	...
	<h3><?php _e('Settings','wpacp'); ?></h3>
	
	<table class="form-table">
		<tr valign="top"> 
			<th scope="row" style="width:130px;"><label for="pollp_status"><?php _e('Status','wpacp'); ?></label></th> 
			<td><input id="pollp_status0" name="pollp_status" type="radio" value="0" <?php checked('0', '0'); ?>  /> <label for="pollp_status0"><?php _e('open','wpacp'); ?></label><br />
			<input id="pollp_status1" name="pollp_status" type="radio" value="1" <?php checked('1', '0'); ?>  /> <label for="pollp_status1"><?php _e('closed','wpacp'); ?></label>
			</td> 
		</tr>
		
		<tr valign="top"> 
			<th scope="row" style="width:130px;"><label for="pollp_expiry_date"><?php _e('Closing vote date','wpacp'); ?></label></th> 
			<td><input id="pollp_expiry" name="pollp_expiry" type="checkbox" value="0" <?php checked('0', '0'); ?>  onclick="wpacp_expiry_date_js()" /> <label for="pollp_expiry"><?php _e('Never','wpacp'); ?></label><br />
			<div id="expiry_date"><input size="12" name="pollp_expiry_date" type="text" id="pollp_expiry_date" placeholder="<?php _e('yyyy/dd/mm','wpacp'); ?>" style="margin-right:5px;" /> <input name="pollp_actual_expiry_date" type="hidden" id="pollp_actual_expiry_date" value="<?php echo date('Y/m/d');?>" /></div>
			</td> 
		</tr>
		
		<tr valign="top"> 
			<th scope="row" style="width:130px;"><label for="pollp_adorder"><?php _e('Display results by...','wpacp'); ?></label></th> 
			<td><input id="pollp_adorder0" name="pollp_adorder" type="radio" value="0" <?php checked('0', '0'); ?>  /> <label for="pollp_adorder0"><?php _e('questions with most positive answers first','wpacp'); ?></label><br />
			<input id="pollp_adorder1" name="pollp_adorder" type="radio" value="1" <?php checked('1', '0'); ?>  /> <label for="pollp_adorder1"><?php _e('questions with most negative answers first','wpacp'); ?></label><br />
			<input id="pollp_adorder2" name="pollp_adorder" type="radio" value="2" <?php checked('2', '0'); ?>  /> <label for="pollp_adorder2"><?php _e('original order of questions','wpacp'); ?></label>
			</td> 
		</tr>
		
		<tr valign="top"> 
			<th scope="row" style="width:130px;"><?php _e('Allow to view results without voting','wpacp'); ?></th> 
			<td><input id="pollp_vresult" name="pollp_vresult" type="checkbox" value="1" <?php checked('1', '0'); ?>  /> <label for="pollp_vresult"><?php _e('Yes','wpacp'); ?></label>
			</td> 
		</tr>
		
		<!--<tr valign="top"> 
			<th scope="row" style="width:130px;"><label for="pollp_url"><?php _e('Poll\'s URL','wpacp'); ?></label></th> 
			<td><input id="pollp_url" size="70" name="pollp_url" class="url" type="text" value="" /></td> 
		</tr>-->
		
		<tr valign="top"> 
			<th scope="row" style="width:130px;"><?php _e('Allow visitors to add questions?','wpacp'); ?></th> 
			<td><input id="pollp_aq" name="pollp_aq" type="checkbox" value="1" <?php checked('1', '0'); ?>  /> <label for="pollp_aq"><?php _e('Yes','wpacp'); ?></label>
			</td> 
		</tr>
	...
	<input type='hidden' name='wpacp_add_submit' />
	...
	<div id="poll_shortcode" <?php echo $displaysc;?>>
		<h3><?php _e('Add this poll to your article','wpacp'); ?></h3>
		<p><?php _e('Copy and paste the "shortcode" below into any page or article you  wish','wpacp'); ?></p>
		<code>[colorvote id="<span id="pollid"><?php echo $pollid;?></span>" style="wpacp-poll"]</code>
	</div>-->
...
$pollid=$edit;
$poll=wpacp_get_poll_id($pollid);
$questions=wpacp_get_questions_poll_id($pollid);
$answers=wpacp_get_answers_poll_id($pollid);
 ?>
<div class="wrap" id="editform">
	<h2><?php _e('Edit Colored Poll - ID '.$pollid,'wpacp'); ?></h2>
...
<div style="float:left;width:70%;">	
	<form name="add_poll" id="add_poll" action="<?php menu_page_url('wpacp-add-poll');?>&edit=<?php echo $pollid;?>" method="post">	
	<input type="hidden" name="pollid" value="<?php echo $pollid; ?>" />
	<table class="form-table">
		<tbody>
		<tr valign="top">
			<th scope="row" style="font-weight:bold;width:100px;"><label for="pollp_title"><?php _e('Title','wpacp');?> <em>*</em></label></th>
			<td><input type="text" size="50" id="pollp_title" name="pollp_title" value="<?php echo $poll[0]['pollp_title'];?>" class="required" placeholder="<?php _e('Your Title...','wpacp');?>" /></td>
		</tr>
		<tr valign="top">
			<th scope="row" style="padding-top:40px;font-weight:bold;width:100px;"><label for="polldescedit"><?php _e('Description','wpacp');?></label></th>
			<td>
	...
	<table class="form-table">
		<tr valign="top"> 
			<th scope="row" style="width:130px;"><label for="pollp_aorder"><?php _e('My answers start by','wpacp'); ?></label></th> 
			<td><input id="pollp_aorder0" name="pollp_aorder" type="radio" value="0" <?php checked('0', $poll[0]['pollp_aorder'] ); ?>  /> <label for="pollp_aorder0"><?php _e('negatives and end with positives','wpacp'); ?></label><br />
			<input id="pollp_aorder1" name="pollp_aorder" type="radio" value="1" <?php checked('1', $poll[0]['pollp_aorder'] ); ?>  /> <label for="pollp_aorder1"><?php _e('positives and end with negatives','wpacp'); ?></label>
			</td> 
		</tr>
	</table>
	...
	<table class="form-table" id="polla_add_table" style="width:320px;margin-left:10px;">
		<tbody id="polla_add_tbody">
			...
			<td class="dragHandle" style="width:16px;padding:0px;padding-bottom:5px;" title="Drag to reorder" >&nbsp;<input type="hidden" name="aorder[]" value="<?php echo $i;?>" /></td>
			<td style="padding:0px;padding-left: 2px;"><input type="button" id="a<?php echo $i;?>" class="deleteRow" value="<?php echo $answer['polla_aid'];?>" style="text-indent:-99999px;" onclick="wpacp_edit_delete_answer_js(this.id);" /><input type="hidden" value="<?php echo $answer['polla_aid'];?>" name="aid[]" /></td>
			</tr>
			<?php $i++; } ?>	
		</tbody>
	</table>
	...
	<p>	
			<input type="hidden" id="aid_del" name="aid_del" value="" /><input type="button" class="button" name="add_answer" onclick="wpacp_add_poll_answer_js(<?php echo $i;?>);" value="<?php _e('Add answer') ?>" /> </p>
	<p><input type="button" class="button" name="reset_colors" value="<?php _e('Reset Colors') ?>" onclick="wpacp_reset_colors_js(<?php echo $i;?>);" /> 
	</p>
	...
	<table class="form-table">
		<tr valign="top"> 
			<th scope="row" style="width:130px;"><label for="pollp_status"><?php _e('Status','wpacp'); ?></label></th> 
			<td><input id="pollp_status0" name="pollp_status" type="radio" value="0" <?php checked('0', $poll[0]['pollp_status'] ); ?>  /> <label for="pollp_status0"><?php _e('open','wpacp'); ?></label><br />
			<input id="pollp_status1" name="pollp_status" type="radio" value="1" <?php checked('1', $poll[0]['pollp_status'] ); ?>  /> <label for="pollp_status1"><?php _e('closed','wpacp'); ?></label>
			</td> 
		</tr>
		
		<tr valign="top"> 
			<th scope="row" style="width:130px;"><label for="pollp_expiry_date"><?php _e('Closing vote date','wpacp'); ?></label></th> 
			<td><input id="pollp_expiry" name="pollp_expiry" type="checkbox" value="0" <?php checked('0', $poll[0]['pollp_expiry'] ); ?>  onclick="wpacp_expiry_date_js()" /> <label for="pollp_expiry"><?php _e('Never','wpacp'); ?></label><br />
			<div id="expiry_date"><input size="12" name="pollp_expiry_date" type="text" id="pollp_expiry_date" placeholder="<?php _e('yyyy/dd/mm','wpacp'); ?>" style="margin-right:5px;" value="<?php echo date('Y/d/m',strtotime($poll[0]['pollp_expiry_date']) );?>" /> <input name="pollp_actual_expiry_date" type="hidden" id="pollp_actual_expiry_date" value="<?php echo $poll[0]['pollp_expiry_date'];?>" /></div>
			</td> 
		</tr>
		
		<tr valign="top"> 
			<th scope="row" style="width:130px;"><label for="pollp_adorder"><?php _e('Display results by...','wpacp'); ?></label></th> 
			<td><input id="pollp_adorder0" name="pollp_adorder" type="radio" value="0" <?php checked('0', $poll[0]['pollp_adorder'] ); ?>  /> <label for="pollp_adorder0"><?php _e('questions with most positive answers first','wpacp'); ?></label><br />
			<input id="pollp_adorder1" name="pollp_adorder" type="radio" value="1" <?php checked('1', $poll[0]['pollp_adorder']  ); ?>  /> <label for="pollp_adorder1"><?php _e('questions with most negative answers first','wpacp'); ?></label><br />
			<input id="pollp_adorder2" name="pollp_adorder" type="radio" value="2" <?php checked('2', $poll[0]['pollp_adorder']  ); ?>  /> <label for="pollp_adorder2"><?php _e('original order of questions','wpacp'); ?></label>
			</td> 
		</tr>
		
		<tr valign="top"> 
			<th scope="row" style="width:130px;"><?php _e('Allow to view results without voting','wpacp'); ?></th> 
			<td><input id="pollp_vresult" name="pollp_vresult" type="checkbox" value="1" <?php checked('1', $poll[0]['pollp_vresult'] ); ?>  /> <label for="pollp_vresult"><?php _e('Yes','wpacp'); ?></label>
			</td> 
		</tr>
		...
		<tr valign="top"> 
			<th scope="row" style="width:130px;"><?php _e('Allow visitors to add questions?','wpacp'); ?></th> 
			<td><input id="pollp_aq" name="pollp_aq" type="checkbox" value="1" <?php checked('1', $poll[0]['pollp_aq'] ); ?>  /> <label for="pollp_aq"><?php _e('Yes','wpacp'); ?></label>
			</td> 
		</tr>
	...
	<input type='hidden' name='wpacp_edit_submit' />
	...
	<div id="poll_shortcode" <?php echo $displaysc;?>>
		<h3><?php _e('Add this poll to your article','wpacp'); ?></h3>
		<p><?php _e('Copy and paste the "shortcode" below into any page or article you  wish','wpacp'); ?></p>[colorvote id="<?php echo $pollid;?>" style="wpacp-poll"]</div>
