...
check_ajax_referer( 'colorvotepoll' );
	global $wpdb,$table_prefix,$wpacp_options,$wpacp_message, $user_identity, $user_ID;
	...
	if($pollid and $pollid>0 ) {
		...
		$question=wpacp_get_questions_qid($qid);
...
//for 'Vote!' Submit	
	...
	if($pollid and $pollid>0) {
		...
		$check_voted = wpacp_check_voted($pollid);
		if($check_voted == 0) {
			...
			for($i=0;$i<$count;$i++){
				...
				if($aid>0) {	
					...
					$answer=wpacp_get_answers_aid($aid);
			...
			if($poll_logging_method == 0 or $poll_logging_method == 1) {
					...
					$vote_cookie = setcookie('wpacp_voted_'.$poll[0]['pollp_id'], $logstring, ( current_time('timestamp') + $cookie_expiry), COOKIEPATH);
			}
			...
			$checkvoted = $logarray;
			$html.=wpacp_display_poll_results($poll[0]['pollp_id'], $checkvoted);
			print($html);
		}
		else{
			$html=wpacp_display_poll_results($pollid, $check_voted);
			print('<div class="wpacp-message">'.$wpacp_message['already_voted'].'</div>'.$html);
		}
	}
...
//For viewing results	
	...
	if($rid and $rid>0 ) {
		$html='';
		$check_voted = wpacp_check_voted($rid);
		$html.=wpacp_display_poll_results($rid, $check_voted);
		print($html);
	}
//For Submitting questions thru results page
	...
	if($raddq and $raddq>0 ) {
		...
		$html='';
		$check_voted = wpacp_check_voted($raddq);
		$html.=wpacp_display_poll_results($raddq, $check_voted);
		print($html);
	}
