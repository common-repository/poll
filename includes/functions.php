...
function wpacp_all_mandatory_questions($pollid) {
    global $wpdb, $table_prefix;
	$table_name = $table_prefix.WPACP_QUESTIONS_TABLE;
	$questions = $wpdb->get_results("SELECT * FROM $table_name WHERE pollq_pid = '$pollid' ORDER BY pollq_order ASC", ARRAY_A);
...
function wpacp_check_voted($poll_id) {
	global $wpacp_options,$wpacp_message;
	$poll_logging_method = intval($wpacp_options['track']);
	switch($poll_logging_method) {
	// Logged By User login, Cookies And IP
		case 0:
			$check_voted_username = wpacp_check_voted_username($poll_id);
			$check_voted_cookie = wpacp_check_voted_cookie($poll_id);
			if( !empty($check_voted_username) and $check_voted_username != 1 ){
				...
				return wpacp_check_voted_ip($poll_id);
			}
			break;
	// Logged By Cookie
		case 1:
			return wpacp_check_voted_cookie($poll_id);
			break;
	// Logged By IP
		case 2:
			return wpacp_check_voted_ip($poll_id);
...
function wpacp_check_voted_username($poll_id) {
...
function wpacp_check_voted_cookie($poll_id) {
	if(!empty($_COOKIE["wpacp_voted_$poll_id"])) {
		$get_voted_aids = explode(',', $_COOKIE["wpacp_voted_$poll_id"]);
	} else {
		$get_voted_aids = 0;
	}
	return $get_voted_aids;
...
function wpacp_check_voted_ip($poll_id) {
	global $wpdb,$table_prefix;
	$log_table = $table_prefix.WPACP_LOGS_TABLE;
	// Check IP From IP Logging Database
	$get_voted_aids = $wpdb->get_col("SELECT pollip_aid FROM $log_table WHERE pollip_pid = $poll_id AND pollip_ip = '".wpacp_get_ipaddress()."' ");
	if($get_voted_aids) {
		return $get_voted_aids;
	} else {
		return 0;
...
if(!function_exists('wpacp_get_ipaddress')) {
	function wpacp_get_ipaddress() {
...
function wpacp_results_cmp($results1, $results2) {	
/*print_r($results1);
die('test');*/
...
function wpacp_addq_send_email($data){
	$admin_email = get_option('admin_email');
	$message='';
	$message.=__("Question Title:  ","wpacp").$data['qtitle'].'<br />'.__("Description:  ","wpacp").$data['qdesc'];
	$url = wpacp_admin_url( array( 'page' => 'wpacp-add-poll' ) );
	$message.='<br />'.__("Edit this Poll :  ","wpacp").'<a href="' . esc_attr( $url ).'&edit='.$data['pollid'].'">'. esc_attr( $url ).'&edit='.$data['pollid'] .'</a>';
	
	add_filter('wp_mail_content_type',create_function('', 'return "text/html";'));
	wp_mail($admin_email, __("New Question added to Poll ID ",'wpacp').$data['pollid'], $message);
}

//get page ID by page slug
function wpacp_get_page_ID_by_slug($page_slug) {
...
function wpacp_poll_is_closed($poll){
...
function wpacp_page_exists_by_slug($pagename)
{
...
//Add Color Polls Admin URL
function wpacp_admin_url( $query = array() ) {
	global $plugin_page;
	if ( ! isset( $query['page'] ) )
